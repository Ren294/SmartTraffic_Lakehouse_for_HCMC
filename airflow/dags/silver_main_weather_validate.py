...
check_file_checktask = SSHOperator(
    ...
    command=check_file_job(
        f"validate/weather/SilverWeatherMainCheckConflicts.py"),
    dag=dag
)
check_conflicts_task = SSHOperator(
    ...
    command=spark_submit(
        "validate/weather/SilverWeatherMainCheckConflicts.py"),
    dag=dag
)
check_file_mergetask = SSHOperator(
    ...
    ssh_hook=ssh_hook,
    command=check_file_job(
        f"validate/weather/SilverWeatherMainMerge.py"),
    dag=dag
)
merge_data_task = SSHOperator(
    ...
    ssh_hook=ssh_hook,
    command=spark_submit(
        "validate/weather/SilverWeatherMainMerge.py"),
    dag=dag
)
