...
def get_merge_info(**context):
    ...
    merge_info = redis_client.lpop("silver_merge_queue_traffic")
...
def prepare_spark_config(**context):
    ...
    config = {
        ...
        'target_branch': 'staging_traffic',
        'date': merge_info['date']
    }
    redis_client = get_redis_client()
    redis_client.rpush(
        "silver_merge_queue_traffic_pre_checked", json.dumps(config))
...
def commit_changes(**context):
    ...
    staging_branch = repo.branch("staging_traffic")

    metadata = json.loads(merge_info['metadata'])

    commit_message = f"Merged traffic data from {
    ...
    try:
        ...
        config = {
            'source_branch': 'staging_traffic',
            'target_branch': 'main',
            'date': merge_info['date']
        }
        redis_client = get_redis_client()
        redis_client.rpush("main_merge_queue_traffic", json.dumps(config))
        return {
            'status': 'success',
            'commit_id': commit.get_commit().id,
            'branch': 'staging_traffic'
        }
...
dag = DAG(
    ...
    description='Merge traffic data from silver branch to staging',
...
merge_data_task = SSHOperator(
    task_id='merge_data',
    ssh_hook=ssh_hook,
    command="""/opt/spark/bin/spark-submit --master spark://spark-master:7077 --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.2.0,org.apache.kafka:kafka-clients:3.2.0,org.apache.hudi:hudi-spark3.2-bundle_2.12:0.15.0,org.apache.hadoop:hadoop-aws:3.3.1,com.amazonaws:aws-java-sdk-bundle:1.11.1026,io.lakefs:hadoop-lakefs-assembly:0.2.4 /opt/spark-apps/validate/traffic/SilverTrafficStagingMerge.py""",
    dag=dag
)
