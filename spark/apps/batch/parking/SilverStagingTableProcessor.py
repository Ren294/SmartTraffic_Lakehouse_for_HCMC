...
class DataProcessor:
    ...
    def read_postgres_table(self) -> DataFrame:
        ...
        return self.spark.read \
            ...
            .option("dbtable", f"parking.{self.table_name}") \
...
class ChangeProcessor:
    def __init__(self, table_name: str):
        ...
        self.spark = create_spark_session(
            ...
            f"SilverStagingParking_{table_name}_processor"
        )
        ...
        try:
            ...
            if changes_df.count() > 0:
                ...
                modified_branch = redis_client.lpop(
                    f"modified_branch_parking_{self.table_name}").decode('utf-8')
                print(f"Modified branch: {modified_branch}")

                path = f"s3a://silver/{modified_branch}/parking/parking_{self.table_name}_modified" \
                    .replace(" ", "")
    ...
    def update_hudi(self) -> None:
        ...
        try:
            ...
            config_str = redis_client.lpop(
                f"parking_modified_{self.table_name}")
            config = json.loads(config_str)

            path = f"s3a://silver/{config['modified_branch']}/parking/parking_{self.table_name}_modified" \
                .replace(" ", "")
