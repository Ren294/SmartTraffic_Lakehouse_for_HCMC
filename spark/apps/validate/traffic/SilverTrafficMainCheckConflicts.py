...
def check_conflicts(spark, source_branch, target_branch, date):
    ...
    if target_df and target_df.count() > 0:
        # Compare records by location_id, timestamp and date to detect conflicts
    ...
    redis_client.rpush("main_merge_queue_traffic_checked",
                       json.dumps(config))
...
if __name__ == "__main__":
    ...
    merge_request_str = redis_client.lpop(
        "main_merge_queue_traffic_pre_checked")
    merge_request = json.loads(merge_request_str)

    spark = create_spark_session(
        lakefs_user["username"], lakefs_user["password"], "SilverTrafficMainCheckConflicts")
    check_conflicts(
